function solve!(calc::Calc; α = 0.0)
    N = length(calc.λ)
...
@memoize function calc_rs(c::Calc, n, p, ψₜ, α)
    ...
    if n == 1
        if c.seed == "0"
            ...
            rf = exp.(+im.*(c.λ[p] .* t .+ c.λ[p]^2 * (1 - 4*α*c.λ[p]) .* x .- π/4))
            sf = exp.(-im.*(c.λ[p] .* t .+ c.λ[p]^2 * (1 - 4*α*c.λ[p]) .* x .- π/4))
        elseif c.seed == "exp"
            # Set up x and t in the proper way
            t = c.box.t .- c.tₛ[p]
            x = c.box.x' .- c.xₛ[p]
            # Compute r and s directly
            A = +c.χ[p] .+ 0.5.*(c.Ω[p].*t .+ c.Ω[p]*(c.λ[p] + α*(1-2*c.λ[p]^2)).*x) .- π/4
            B = -c.χ[p] .+ 0.5.*(c.Ω[p].*t .+ c.Ω[p]*(c.λ[p] + α*(1-2*c.λ[p]^2)).*x) .- π/4
        ...
        r1, s1 = calc_rs(c, n-1, 1, ψₜ, α)
        r2, s2 = calc_rs(c, n-1, p+1, ψₜ, α)
